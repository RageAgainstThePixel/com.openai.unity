...
namespace OpenAI.Files
{
    ...
    public sealed class FilesEndpoint : OpenAIBaseEndpoint
    {
            ...
            async Task<bool> InternalDeleteFileAsync(int attempt)
            {
                ...
                if (!response.Successful)
                {
                        ...
                        response.Body.Contains(fileProcessing))
                    {
                        ...
                        await Awaiters.DelayAsync(1000 * attempt++, cancellationToken).ConfigureAwait(true);
        ...
        {
            var file = await GetFileInfoAsync(fileId, cancellationToken);
            return await Rest.DownloadBytesAsync(GetUrl($"/{file.Id}/content"), new RestParameters(client.DefaultRequestHeaders, progress, debug: EnableDebug), cancellationToken);
        }
    }
}
