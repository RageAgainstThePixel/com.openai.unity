...
namespace OpenAI
{
    ...
    public class SearchEndpoint : BaseEndPoint
    {
        ...
        {
            var jsonContent = JsonConvert.SerializeObject(searchRequest, Api.JsonSerializationOptions);
            var response = await Api.Client.PostAsync(GetEndpoint(engine), jsonContent.ToJsonStringContent());

            if (response.IsSuccessStatusCode)
            {
                var resultAsString = await response.Content.ReadAsStringAsync();
                var searchResponse = JsonConvert.DeserializeObject<SearchResponse>(resultAsString);
        ...
        {
            var results = await GetSearchResultsAsync(new SearchRequest(query, documents), engine);
            var kvp = results.ToList().OrderByDescending(kv => kv.Value).FirstOrDefault();
            return new Tuple<string, double>(kvp.Key, kvp.Value);
        }
    }
}
